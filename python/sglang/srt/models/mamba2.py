...
class Mamba2DecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        # Mamba2 SSM mixer - create cache params from config
        ...
        # All layers in Mamba2 use the SSM mixer
    ...
    def forward(
        ...
        # Pure Mamba2 block: pre-norm (with fused residual add) -> SSM mixer.
        # There is no MLP sublayer in Mamba-Codestral; each layer is norm + mixer.
        if residual is None:
        ...
        # Mamba2 mixer via the Mamba2 attention backend, which manages the
        # per-layer conv/ssm state cache and forward metadata. use_triton_causal_conv
        # keeps the causal conv on a portable Triton kernel (required off-CUDA, e.g. XPU).
...
class Mamba2ForCausalLM(nn.Module):
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]):
        ...
        for name, loaded_weight in weights:
            ...
            # Map HuggingFace Mamba2 checkpoint names to SGLang module names:
            #   backbone.           -> model.
            #   backbone.embeddings -> model.embed_tokens
            #   backbone.norm_f     -> model.norm
            #   ...mixer.A_log      -> ...mixer.A
            # (lm_head.weight is top-level and already maps directly.)
        ...
        # Log any parameters that weren't loaded
