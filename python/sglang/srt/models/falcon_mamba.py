...
class FalconMambaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        # Drive the mixer through the (shared) Mamba2 attention backend, which
        # owns the per-layer conv/ssm state cache and forward metadata.
        # use_triton_causal_conv keeps the causal conv on a portable Triton
        # kernel (required off-CUDA, e.g. XPU).
...
class FalconMambaForCausalLM(nn.Module):
    ...
    def load_weights(self, weights: Iterable[Tuple[str, torch.Tensor]]):
        ...
        for name, loaded_weight in weights:
            # Map HuggingFace Falcon-Mamba checkpoint names to SGLang modules:
            #   backbone.            -> model.
            #   backbone.embeddings  -> model.embed_tokens
            #   backbone.norm_f      -> model.norm
            # (lm_head.weight is top-level and maps directly.)
            # NOTE: unlike Mamba2, keep `A_log` as-is — the mixer stores A_log
            # and computes A = -exp(A_log) at runtime.
