...
class MambaMixer1(nn.Module):
    ...
    def _forward_prefill(
        ...
        *,
        ...
        # The causal-conv kernel combines the input, weights and the persistent
        # conv-state cache in one op, so all three must share a dtype. The cache
        # dtype (SGLANG_MAMBA_CONV_DTYPE, default bf16) is independent of the
        # model dtype, so for a model whose activations resolve to a different
        # float type (e.g. an fp32 checkpoint auto-cast to fp16) we run the conv
        # in the cache dtype and cast the result back to the activation dtype.
        act_dtype = x.dtype
